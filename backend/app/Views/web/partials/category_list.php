...
<?php if (!empty($categoryItems)): ?>
    <div class="mt-5 md:mt-8 pt-1 pb-4 overflow-x-auto" style="scrollbar-width:none;-ms-overflow-style:none;">
        <div class="flex min-w-max gap-1 md:gap-4" style="-webkit-overflow-scrolling:touch;">
            <?php foreach ($categoryItems as $category): ?>
                <a
                    href="<?= base_url('/category/' . $category['slug']) ?>"
                    class="flex w-[85px] md:w-[118px] shrink-0 flex-col items-center justify-center rounded-sm md:rounded-[28px] px-2 py-3 md:px-4 md:py-5 text-center shadow-primary-100/35 transition hover:-translate-y-1 hover:shadow-lg"
                >
